...
def get_marca(id):
    ...
    marca = Product.query.filter_by(brand=get_m).paginate(page=pagina, per_page=4)
    marcas = Brand.query.join(Product, (Brand.id == Product.brand_id)).all()
    categorias = Category.query.join(Product, (Category.id == Product.category_id)).all()
...
def get_categoria(id):
    ...
    get_cat_prod = Product.query.filter_by(category=get_cat).paginate(page=pagina, per_page=4)
    marcas = Brand.query.join(Product, (Brand.id == Product.brand_id)).all()
    categorias = Category.query.join(Product, (Category.id == Product.category_id)).all()
